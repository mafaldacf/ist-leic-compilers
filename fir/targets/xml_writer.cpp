...
static std::string qualifier_name(int qualifier){
  ...
  if (qualifier == '?') return "imported";
  if (qualifier == 'p') return "private";
...
void fir::xml_writer::do_while_finally_node(fir::while_finally_node * const node, int lvl) {
  ...
  node->condition()->accept(this, lvl + 4);
  closeTag("condition", lvl + 2);
  openTag("doInstruction", lvl + 2);
  node->doInstruction()->accept(this, lvl + 4);
  closeTag("doInstruction", lvl + 2);
  openTag("finallyInstruction", lvl + 2);
  node->finallyInstruction()->accept(this, lvl + 4);
  closeTag("finallyInstruction", lvl + 2);
  closeTag(node, lvl);
}
...
void fir::xml_writer::do_if_node(fir::if_node * const node, int lvl) {
  ...
  node->condition()->accept(this, lvl + 4);
  closeTag("condition", lvl + 2);
  openTag("thenInstruction", lvl + 2);
  node->instruction()->accept(this, lvl + 4);
  closeTag("thenInstruction", lvl + 2);
  closeTag(node, lvl);
}
...
void fir::xml_writer::do_if_else_node(fir::if_else_node * const node, int lvl) {
  ...
  closeTag("thenInstruction", lvl + 2);
  openTag("elseInstruction", lvl + 2);
  node->elseInstruction()->accept(this, lvl + 4);
  closeTag("elseInstruction", lvl + 2);
  closeTag(node, lvl);
}
...
void fir::xml_writer::do_function_definition_node(fir::function_definition_node * const node, int lvl) {
 ...
 _symtab.push(); // scope of args
  ...
  if (node->defReturnValue())
    node->defReturnValue()->accept(this, lvl + 4);
...
void fir::xml_writer::do_block_node(fir::block_node * const node, int lvl) {
  ...
  if (!_inPrologue)
    _symtab.push(); // scope of main block or epilogue declarations
  ...
  if (!_inPrologue)
    _symtab.pop(); // scope of main block or epilogue declarations
...
void fir::xml_writer::do_indexation_node(fir::indexation_node * const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  openTag(node, lvl);
  openTag("base", lvl + 2);
